...
class Line {
  ...
  public:
    ...
    std::optional<std::pair<Point, Point>> GetYRayHorizontalLineIntersectPoint(double y) {
        if (this->start.GetY() == this->end.GetY() && y == this->start.GetY()) {
            return std::optional<std::pair<Point, Point>>({this->start, this->end});
        }
        return std::optional<std::pair<Point, Point>>();
    }
     ...
     * intersection.
     */
    std::optional<std::vector<Point>> GetIntersectPoint(Line other) {
        ...
        if (Between<double>(std::floor(x * 1e6), std::floor(this_min_x * 1e6), std::floor(this_max_x * 1e6)) &&
            ...
            return std::optional<std::vector<Point>>({std::vector<Point>({Point({x, y})})});
        }

        return std::optional<std::vector<Point>>();
    }

    bool IsHorizontalLine() { return this->start.GetY() == this->end.GetY(); }
};
